...
function tabber() {
	...
	                                $s_bet = $('#gbs_bet')
		...
		        $('<li>').append($('<a>').text('Bot').attr('href', '#Nixsy9')).appendTo('.tabs');
};
...
function create_ui() { // creates most of the gui stuff
	...
	var $numz4 = $('<p style="margin-right:15px;border:1px solid; border-color: #505050;" class="rlabel">Đ</p>');
	  $row3.append($label5);
	...
	if ($multiplier !== undefined && $steps !== undefined)
		...
		if ($.isNumeric($multiplier.val()) && $.isNumeric($steps.val()) && $.isNumeric($('#pct_bet').val())) {
			...
			for (i = 0; i < $steps.val(); i++) {
				total += parseFloat($('#pct_bet').val()) * mult; //total = total + $('#pct_bet').val() * mult;
